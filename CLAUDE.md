...
- `PasskeyIntegrationTest` uses `Fixtures\FixedKeyAuthenticator` (a fixed P-256 key), NOT the raw emulator — `pronin/webauthn-emulator` emits unpadded EC coords (`openssl_pkey_get_details` strips leading zeros), so ~1/128 keys are 31 bytes and `web-auth/cose-lib` rejects them (500). Don't swap back to the bare emulator. A fix was submitted upstream to `pronin/webauthn-emulator`.
