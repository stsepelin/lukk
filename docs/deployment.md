...
lukk's own `/auth/*` routes always render JSON errors (it forces JSON on them); if the same app also serves web routes, your *own* `auth:api` routes need the same — see [Installation](installation.md#wiring-the-guard).
